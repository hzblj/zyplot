...
/**
 * Distributions side by side — the spread of discount depth per retailer, not
 * just each retailer's average.
 *
 * A grouped bar chart of averages hides exactly the thing worth seeing: whether
 * a chain discounts consistently or swings between token cuts and clear-outs.
 * Outliers are drawn as individual points rather than folded into the whiskers,
 * because in this data an outlier is usually a real clearance, not noise.
 */

export type BoxplotChartProps = ChartBaseProps & {
	format?: ChartNumberFormat;
	groups: ChartBoxplotGroup[];
	labels: {
		max: string;
		median: string;
		min: string;
