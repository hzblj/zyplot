...
export const marketingStyles = tv({
	slots: {
		...
		/**
		 * A row on desktop, two rows on a phone.
		 *
		 ...
		 * right edge and take the whole page's horizontal scroll with it. Below
		 * 560px it fills the column instead and the command moves under the tabs.
		 */
		install:
			"flex items-center gap-2 rounded-full border border-border-secondary bg-surface-secondary py-2.5 pl-3 pr-2 max-[560px]:w-full max-[560px]:flex-wrap max-[560px]:gap-y-1.5 max-[560px]:rounded-[22px] max-[560px]:px-3",
		/**
		 ...
		 */
		installCommand:
			"whitespace-nowrap px-1 text-[13px] text-content-secondary max-[560px]:min-w-0 max-[560px]:flex-1 max-[560px]:overflow-x-auto max-[560px]:px-0.5",
		installCopy:
		...
		/** Its own row on a phone, so the command below keeps the copy button. */
		installTabs: "flex items-center gap-0.5 max-[560px]:w-full",
