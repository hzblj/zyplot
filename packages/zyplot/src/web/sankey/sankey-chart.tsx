...
/**
 * Where a quantity goes as it moves through stages — leaflets discovered →
 * downloaded → rasterised → extracted → reviewed, with the drop-off at each hop
 * visible as the width that fails to continue.
 *
 * It answers one question no other form here answers well: *where* volume is
 * lost, not just how much. It needs a genuine flow to be worth the space — a
 * sankey over two stages is a bar chart drawn expensively.
 */
...
export const SankeyChart: FC<SankeyChartProps> = ({
	...
	const option = useMemo(() => {
		...
		const colorById = new Map(
			nodes.map((node, index) => [node.id, seriesColor(tokens, node, index)]),
		);
			...
			series: [
				{
					...
					// Named by label, not by id: ECharts matches links to nodes on `name`.
					links: links.map((link) => ({
						source:
							nodes.find((node) => node.id === link.source)?.label ??
							link.source,
						target:
							nodes.find((node) => node.id === link.target)?.label ??
							link.target,
						value: link.value,
					})),
					nodeGap: 10,
