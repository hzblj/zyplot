    ...
    {Array.from({length: count}, (_value, index) => {
      ...
      return (
        <div className="relative h-2.5 w-full" key={index}>
          ...
          />
          <Skeleton className="absolute top-0 size-2.5 rounded-full" style={{left: `calc(${start}% - 5px)`}} />
          <Skeleton className="absolute top-0 size-2.5 rounded-full" style={{left: `calc(${end}% - 5px)`}} />
        </div>
      )
    })}
  </div>
